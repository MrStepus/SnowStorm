...
{
    public event Action<int, int> ItemsAdded;
    public event Action<int, int> ItemsRemoved;
    ...
    public AddItemsToInvenroryGridResult AddItems(int itemId, int amount = 1)
    {
    ...
    public AddItemsToInvenroryGridResult AddItems(Vector2Int slotCords, int itemId, int amount = 1)
    {
    ...
    public RemoveItemsFromInventoryGridResult RemoveItems(int itemId, int amount = 1)
    {
    ...
    public RemoveItemsFromInventoryGridResult RemoveItems(Vector2Int slotCords, int itemId, int amount = 1)
    {
    ...
    private int AddToFristAveilableSlots(int itemId, int amount, out int remainingAmount)
    {
        var itemsAddedAmount = 0;
        remainingAmount = amount;
    ...
    public int GetItemSlotCapasity(int itemId)
    {
        var config = ItemDatabase.GetConfig(itemId);
    
        // Если конфиг найден - берем его maxStack, иначе дефолт 18
        return config != null ? config.maxStack : 18;
    }
